...
func (db Database) CreateCompany(ctx *gin.Context) {
	//vérifier les privilèges ou les autorisations de l'utilisateur qui effectue une action dans l'application.
...
func (db Database) ReadCompanies(ctx *gin.Context) {
	...
	//strconv.Atoi(...) : Cette fonction convertit la valeur de chaîne du paramètre de page en un entier
/*
 Cette ligne extrait le paramètre de requête nommé "page" à partir du contexte Gin (ctx).
  Si ce paramètre n'est pas présent dans la requête, elle utilise la valeur 
 par défaut spécifiée par constants.DEFAULT_PAGE_PAGINATION.
...
/*si vous avez une pagination avec une limite de 10 éléments par page et que vous êtes sur la page 3, l'offset serait (3 - 1) * 10 = 20, ce qui signifie que vous devez 
ignorer les 20 premiers éléments avant de récupérer les éléments pour la page 3.*/
	// Generate offset
	offset := (page - 1) * limit
